...
class AttrData(object):
    ...
    def build_from_dict(self, attr_dict):
        # if attribute is forwarded the only allowed keyword is "label"
        self.forwarded = attr_dict.pop('forwarded', False)
        if self.forwarded and not attr_dict.keys() in [[],['label']]:
            self.__throw_exception("Invalid attribute definition keywords: "
                "only \"label\" is allowed for forwarded attributes")
    ...
    def __create_user_default_attr_prop(self, extra_info):
        ...
        for k, v in extra_info.items():
            ...
            if hasattr(p, method_name):
                ...
                p.set_delta_t(str(v))
            elif not k_lower in ('display level', 'polling period', 'memorized', 'forwarded'):
    ...
    def from_attr_info(self, attr_info):
        ...
        # if treating a forwarded attribute we must check that the only
        # keyword provided is "label" (if any)
        try:
            forwarded = extra_info.get('forwarded', False)
        except Exception:
            throw_ex("Wrong forwarded value for attribute %s in class %s."
                     "Allowed values are boolean values True or False")
        if forwarded:
            self.forwarded = True
            keywords = extra_info.copy()
            keywords.pop('forwarded')
            if not keywords.keys() in [[],['label']]:
                throw_ex("Invalid attribute definition keywords: only "
                         "\"label\" is allowed for forwarded attributes")
            self.attr_class = FwdAttr
        else:
            ...
            try:
                ...
                throw_ex("Wrong memorized value. for attribute %s in class %s."
                         "Allowed values are the strings \"true\", \"false\" and "
                         "\"true_without_hard_applied\" (case insensitive)")
            if memorized == "true":
                self.memorized = True
